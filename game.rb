...
class Game
  ...
  def displpay_score(current_roll_score, current_turn_score, total_score)
    puts "Score in this round: #{current_roll_score}"
    puts "Total score: #{total_score + current_turn_score}"
  end

  def handle_invalid_input(continue_rolling)
    unless continue_rolling.downcase == 'y' || continue_rolling.downcase == 'n'
      begin
  ...
  def play_game
    ...
    loop do
      ...
      loop do
        if non_scoring_dice.positive?
          ...
          if continue_rolling == 'y'
            ...
            current_turn_score = current_roll_score.zero? ? 0 : current_turn_score += current_roll_score
            displpay_score(current_roll_score, current_turn_score, current_player.score)
            ...
            continue_rolling = 'n' if current_roll_score.zero?
          end
        end
        break if continue_rolling == 'n' || non_scoring_dice.zero?
      end

      current_player.score += current_turn_score if current_player.score != 0 || current_turn_score >= ELIGIBILITY_SCORE
      ...
      break if last_round && current_player.id == @players.size

      # check for the last round
      last_round = true if current_player.id == @players.size && @players.find { |p| p.score >= MIN_WINNING_SCORE }
